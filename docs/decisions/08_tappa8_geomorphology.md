...
- Aluminium is now IMPLEMENTED (S10, bauxite/volcanic/`mundane_only`) — the remaining
  ...
  wanted.
- Vértice material assignments for marble / sedimentary_limestone / granite — the three
  rock types now exist (S8) but have no domain/verb/tier entry in `resources.py`'s
  `VERTICE_MATERIALS` yet; genuinely undesigned, not just unimplemented. Routed to the
  Scenario chat (2026-08-20) rather than continued here — this is a world-logic "nature of
  each Vértice" question, not a geomorphology one; a factual handoff on the 3 new rocks'
  real mineral/physical properties was prepared for that discussion
  (`docs/reference/tappa8_new_rocks_vertice_handoff.md`).
